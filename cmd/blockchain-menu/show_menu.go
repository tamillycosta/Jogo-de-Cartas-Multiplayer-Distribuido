...
func showMenu() {
	...
	fmt.Println(strings.Repeat("═", 80))
	fmt.Println()
	fmt.Println("   RELATÓRIOS GERAIS:")
	fmt.Println("    [1] Resumo do Sistema")
	fmt.Println("    [2] Listar Todos os Pacotes")
	fmt.Println()
	fmt.Println("   CONSULTAS DETALHADAS:")
	fmt.Println("    [3] Detalhes de um Pacote")
	fmt.Println("    [4] Histórico de um Jogador")
	fmt.Println("    [5] Histórico de uma Carta")
	fmt.Println("    [6] Detalhes de uma Transação")
	fmt.Println()
